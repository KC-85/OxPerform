...
from apps.events.oxfordshire.south.models import Event, EventStatus, EventCategory, Venue


class SouthOxonViewsTestCase(TestCase):
    def setUp(self):
    ...
    def test_upcoming_events_list_shows_only_approved_public_active_venue_upcoming(self):
        url = reverse("southoxon:upcoming_events")
    ...
    def test_past_events_list_shows_only_past_approved_public_active_venue(self):
        url = reverse("southoxon:past_events")
        resp = self.client.get(url)
        self.assertEqual(resp.status_code, 200)
    ...
    def test_category_view_filters_by_category(self):
        url = reverse("southoxon:category_events", kwargs={"category": "open_mic"})
        resp = self.client.get(url)
        self.assertEqual(resp.status_code, 200)
    ...
    def test_event_detail_200_for_approved_public_active_venue(self):
        url = reverse("southoxon:event_detail", kwargs={"slug": self.event_upcoming_ok.slug})
        resp = self.client.get(url)
        self.assertEqual(resp.status_code, 200)
        self.assertContains(resp, self.event_upcoming_ok.title)

    def test_event_detail_404_for_pending(self):
        url = reverse("southoxon:event_detail", kwargs={"slug": self.event_pending.slug})
        resp = self.client.get(url)
        self.assertEqual(resp.status_code, 404)

    def test_event_detail_404_for_not_public(self):
        url = reverse("southoxon:event_detail", kwargs={"slug": self.event_private.slug})
        resp = self.client.get(url)
        self.assertEqual(resp.status_code, 404)

    def test_event_detail_404_for_inactive_venue(self):
        url = reverse("southoxon:event_detail", kwargs={"slug": self.event_inactive_venue.slug})
        resp = self.client.get(url)
        self.assertEqual(resp.status_code, 404)